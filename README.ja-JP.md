...
[English](./README.md) · [中文文檔](./README.zh-CN.md) · [日本語](./README.ja-JP.md)
...
| インストール | 使い方 | コントリビュート |
|---|---|---|
| [インストール](#インストール)<br>[`skills` CLI（npx）](#方法-a--skills-clinpx)<br>[Claude Code プラグインマーケットプレイス](#方法-b--claude-code-プラグインマーケットプレイス)<br>[Releases のバージョン固定 `.zip`](#方法-c--releases-のバージョン固定-zip)<br>[手動コピー](#方法-d--プロジェクトへ手動コピー)<br>[Git submodule](#方法-e--git-submodule) | [互換性](#互換性)<br>[Skill とは？](#skill-とは) | [コントリビュート](#コントリビュート)<br>[謝辭](#謝辭)<br>[ライセンス](#ライセンス) |
...
**カテゴリ:** Web 動畫 / プレゼンテーションエンジニアリング  
**用途:** スクリプト、記事、レッスン、製品デモ、トークを、シネマティックな動畫として畫面録畫できるクリック駆動の 16:9 Web プレゼンテーションに変換します。

`web-video-presentation` は、動畫製作環境のように振る舞う録畫対応の Vite + React + TypeScript プレゼンテーションを構築します。このワークフローでは、生の記事をナレーションスクリプトに変換し、スクリプトの各ビートをフルスクリーンシーンにマッピングし、必要なチェックポイントで一時停止し、ビジュアルアウトラインの承認後にオプションでナレーション音聲を合成できます。

主な特徴:

- 安定した畫面録畫のため、ビューポートに合わせてスケールする固定 1920×1080 ステージ
- クリック / キーボード駆動の `(chapter, step)` カーソル、ビジュアルステップごとに 1 つのナレーションビート
- スクリプト、テーマ、アウトライン、実裝モード、オプションの音聲に対する厳格なコラボレーションチェックポイント
- 録畫中もステージをクリーンに保つ、ホバー時のみ表示される進捗コントロール
- **23 種類のビルトインテーマ**（`midnight-press` から `swiss-ikb` まで）を備えたテーマトークンアーキテクチャ。エディトリアル、ターミナル、エンジニアリング、スイス國際主義など、各テーマが獨自のデザインシグネチャを持つ
- **プラガブル TTS**：プロバイダー非依存の音聲ランナー。**2 つのビルトインプロバイダー**（MiniMax `mmx-cli` + curl 経由の OpenAI TTS）を同梱し、ElevenLabs / edge-tts / Azure / Google Cloud / macOS `say` 用のコピペ可能なスニペットも付屬
- 再利用可能なステージプリミティブと録畫ガイダンスを備えた Vite + React + TypeScript プロジェクトのスキャフォールド
...
`web-design-engineer` は、AI が生成する Web 成果物を単に機能するだけのものから、洗練され、意図的で、視覚的に印象に殘るフロントエンド作品へと変えます。エージェントをデザインエンジニアとして扱い、まず製品コンテキストを理解し、次にデザインシステムを宣言し、初期 v0 を示し、フルエクスペリエンスを構築し、結果を検証します。

主な特徴:

- 要件 → コンテキスト → デザインシステム → v0 → フルビルド → 検証という 6 ステップのデザインワークフローを定義
- アンチクリシェのブロックリストとより強いビジュアル判斷力で、一般的な AI UI パターンを超える
- **デザインディレクション・アドバイザー（6 學派の差別化された 3 択推薦）と、anchor 付きスタイルレシピ 25 種類**（Linear / Aesop / Pentagram / Bloomberg / Stripe Press / Mid-Century など）を內蔵。各レシピにはコピー可能なパレット、タイポグラフィ、シグネチャムーブ、アンチパターンが揃っており、デザインシステム宣言にそのまま貼り込める
- HTML / CSS / JavaScript / React プロトタイプをカバーし、レスポンシブレイアウト、モーション、インタラクションの仕上げに関するガイダンスを提供
- インライン React + Babel、CSS トークン、`oklch()` カラー処理、コンテナクエリ、reduced-motion 対応の実裝ルールを含む
...
**カテゴリ:** 畫像生成 / プロンプトエンジニアリング  
**用途:** ポスター、UI モックアップ、製品ビジュアル、インフォグラフィック、學術図、技術図、コミック、アバター、ストーリーボード、ブランディングボード、畫像編集ワークフロー。

`gpt-image-2` は、GPT Image 2 および OpenAI 互換の畫像 API に特化した畫像生成スキルです。完全にローカルな Garden での生成、ホストネイティブの畫像ツールへの委譲、畫像ツールがない場合のプロンプトのみのアドバイザーモードなど、異なるエージェント環境で動作するように設計されています。
...
- 専用のワークフローとスクリプトを通じて、畫像生成と畫像編集の両方をカバー
- Garden モードでは、再利用、レビュー、バージョン管理のためにプロンプトと生成畫像を `garden-gpt-image-2/` 配下に保存
...
`kb-retriever` は、慎重で段階的な検索を中心に構築されたローカル知識ベースのリトリーバです。ファイル全體を読み込むのではなく、階層的なインデックスファイルをたどり、候補集合を絞り込み、複雑なファイルタイプを正しく処理し、出典付きで回答します。

主な特徴:

- 階層化された `data_structure.md` ファイルを使用して、コンテンツを検索する前に知識ベース內を移動
...
| D | [`git clone` 後の手動コピー](#方法-d--プロジェクトへ手動コピー) | スキル自體のローカルでのハック | ❌（`main` を追跡） |
...
> 上記の各スキルの「Links」エントリの下にある「Download」行を參照してください。
> これらはリリースパイプラインによって自動的に同期されます。

### 方法 D · プロジェクトへ手動コピー

リポジトリをクローンし、必要なスキルフォルダをコピーします。スキル自體を
...
[Anthropic のリファレンスリポジトリ](https://github.com/anthropics/skills) を參照してください。
...
- **[Claude Design](https://www.anthropic.com/news/claude-design-anthropic-labs)** - `web-design-engineer` のインスピレーションとなったシステムプロンプト。オリジナルは參考のため [`dist/prompt/claude-design-system-prompt.md`](./dist/prompt/claude-design-system-prompt.md) に保存されています。
- より広い OSS スキルコミュニティ - さらなる発見のために [`travisvn/awesome-claude-skills`](https://github.com/travisvn/awesome-claude-skills) と [`obra/superpowers`](https://github.com/obra/superpowers) を參照してください。
