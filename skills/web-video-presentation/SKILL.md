...
口播音频。产出物 = Vite + Angular (Signals) + TS 项目 + 按章节切分的音频。
...
└── presentation/       # 脚手架产出的 Vite + Angular + TS 项目
    ├── src/app/chapters/<NN>-<id>/
    │   ├── <Chapter>.component.ts  # 视觉实现
    │   ├── <Chapter>.component.css # 专属样式 (以 tokens 变量为主)
    │   ├── layout.json             # 存储拖拽缩放坐标、内联文本与讲者备注
    ...
    │   └── tts-providers/          # 每 provider 一个 .sh
    │       ├── minimax.sh
    │       └── openai.sh
    ├── audio-segments.json         # extract 产出
    └── src/assets/audio/<id>/<N>.mp3 # 可选：合成的音频
```

> **关键**：`narrations.ts` 是 step 数和音频合成的**唯一真相源**。
> 章节组件中的 `step` 信号输入的最大值 N + 1 必须等于
> `narrations.length`。这保证 5 处地方（script / outline / 章节代码 /
> chapters.ts / 音频文件）永远不会漂。此外，`layout.json` 保存了可视化
> 元素的坐标与内容，让编辑面板与画面的拖拽修改可以即时同步持久化。
...
接下来一次对齐 5 件事：
  ...
  5. 开发模式选哪个？
     ...
     **第 1 章无论哪种模式都必须主线程做完 + 用户验收**（强制 anchor）。
     ...
     D) 非工程师引导模式
        我来问你几个问题，替你处理所有指令和环境设定。
        你只需要：回答问题、确认画面、最后按下录影键。
        ⚠️ 每个指令执行前我都会用白话说明「我现在要做什么、为什么」。
```
...
- **主题必须明确**才进入 Phase 2。用户说"主题你帮我选" → 取你推荐的第 1 个，
  **告诉用户你选了什么、为什么**，给反悔机会
- 模式选定 → 进 Phase 2
- **选了 D（非工程师引导模式）→ 进 Phase 2 时全程遵守非工程师规则**（见下方「2.0 非工程师引导规则」）

---

## Phase 2 —— 网页开发

### 2.0 非工程师引导规则（仅选了模式 D 时适用）

用户选了「非工程师引导模式」时，**整个 Phase 2 + Phase 3 + Phase 4 全程遵守以下规则**：
...
**指令执行后**，主动确认：
- 成功 → "✓ 完成。你有没有看到 <预期画面/输出>？"
- 失败 → 把错误翻译成中文，说明原因和修复步骤，**不要直接贴英文报错叫用户自己看**

**遇到环境问题**（没装 Node / Python / ffmpeg 等）时：
1. 告诉用户「你的电脑缺少 X，是 <功能> 需要的工具」
2. 给出一键安装的指令（Windows / macOS 分开列）
3. 安装完帮用户验证是否成功再继续

**TTS 合成（Phase 3）** 优先引导用户用 `edge-tts`（免费、无 key）：
```
要合成语音，最简单的方式是用 edge-tts（微软出品，完全免费，不需要注册帐号）。
只需要执行一条安装指令：
  pip install edge-tts
安装完我来帮你一键合成全部语音。
```

**录屏（Phase 4）** 主动引导用户操作，按平台区分：
- Windows：「按 Win + G 打开 Xbox Game Bar → 点录制按钮」
- macOS：「按 Cmd + Shift + 5 → 选录制选定区域」
...
要不要合成音频做"自动播放录屏"？
  ✓ 合成 → 扫所有章节的 narrations.ts 出 audio-segments.json，
           ...
           内置三个 provider：
             • edge-tts（默认，免费，pip install edge-tts，无需 API key）✅ 推荐新手
             • minimax (mmx-cli)  —— 中文音色更稳，要 MiniMax API key
             • openai  (OPENAI_API_KEY) —— curl-based，多数已有 key
           其它后端 (ElevenLabs / macOS say 离线 / Azure / Google)
           见 scripts/tts-providers/README.md 的现成片段。
...
npm run synthesize-audio                       # 默认 edge-tts（免费，无需 API key）
# 或用内置 minimax（中文音色更稳，要 mmx-cli + API key）：
...
| [`references/AUDIO.md`](references/AUDIO.md) | Phase 3 才读 | provider-agnostic 音频合成流程、内置 edge-tts（默认免费）/ minimax / openai 用法、换 provider 路径、故障排查 |
| [`templates/scripts/tts-providers/README.md`](templates/scripts/tts-providers/README.md) | 换 / 加 TTS provider 时 | 三函数契约 + 内置 3 个 (edge-tts / minimax / openai) + 5 种现成代码片段（ElevenLabs / macOS say / Azure / Google） |
