...
`web-video-presentation` 帮 Agent 构建一种 Vite + Angular + TypeScript 演示：它看起来不是传统幻灯片，而更像为录屏设计的视频舞台。每次点击推进一个口播节拍，每一步独占 1920×1080 舞台，进度 UI 平时隐藏，只有悬浮时出现，方便录出干净画面。
...
生成的 `presentation/` 是普通 Vite + Angular + TypeScript 项目。启动后用录屏工具录制 16:9 舞台即可。
