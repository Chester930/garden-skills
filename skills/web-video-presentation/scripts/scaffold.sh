...
echo "▸ 複製 Angular + Vite 專案模板..."
# Copy all files from templates to target, ensuring dotfiles are copied too
cp -r "$TEMPLATES"/. "$TARGET"/

# Copy selected theme tokens
mkdir -p "$TARGET/src/styles"
cp "$THEME_TOKENS" "$TARGET/src/styles/tokens.css"

# Save active theme bookmark
echo "$THEME" > "$TARGET/.theme"

cd "$TARGET"
echo "▸ 安裝專案依賴（可能要等一會，請保持網路暢通）..."
npm install >/dev/null 2>&1

# Typecheck verification
echo "▸ 跑 typecheck 驗證專案類型..."
