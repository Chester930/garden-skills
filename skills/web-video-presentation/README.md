...
`web-video-presentation` helps an agent build a Vite + Angular + TypeScript presentation that behaves like a video production surface rather than a slide deck. Each click advances one narration beat, each step owns the whole 1920×1080 stage, and the progress UI stays hidden unless hovered so the output is clean for screen recording.
...
Checkpoint A2  Outline approval + development mode
   |
Phase 2    Build the Vite / Angular / TS presentation
   |
...
The generated `presentation/` project is a normal Vite + Angular + TypeScript app. Run it like any other Vite project, then record the 16:9 stage with your screen recorder.
