...
視頻章節規劃的產出文件。**用戶可以直接編輯**，所以格式必須人類友好
（用 markdown 不用 JSON / YAML）。

!重要：閱讀此文件後必須繼續閱讀 [`CHAPTER-CRAFT.md`](CHAPTER-CRAFT.md) 的全部內容，了解對網頁效果的真實需求，然後再開始編寫 outline

> ## ⚠️ outline 是開發計劃，不是視覺規劃
>
> outline 只規劃**節奏 + 內容 + 信息密度**：
>
> - 章節切分 / 每章 step 數 / 每步估時
> - 每步屏幕內容（hero / 標語 / 數據 / 列表項）
> - 章節級**信息池**（從 article 抽的數字 / 引用 / 案例 / 標籤）
>
> **outline 裏的 step 數是初始預估**。最終 step 數以章節實現時的
> `narrations.ts` 為準——後者既是 step 數源，也是音頻合成源
> （詳見 [`CHAPTER-CRAFT.md`](CHAPTER-CRAFT.md) 「代碼層最小約束」+
...
> - **`article.md`**（如有）—— 決定**畫面信息密度**：每章首段抽**信息池**
...
> **關於時長**：outline 裏**只**寫 step 的 `(~Ts)` 口播估時（音畫對齊
...
| 字段 | 必填 | 說明 |
|---|---|---|
| **主題** | ✓ | Checkpoint Plan 必須已選定。chapter agent 實現時按主題顏色 / 字體 token 走，動畫 / 節奏 / 視覺演示由章節自由發揮 |
...
| `<id>` | **小寫 + 連字符**。會成爲 React `key` / 文件夾名 (`src/chapters/0N-<id>/`) / 音頻子目錄 (`public/audio/<id>/`) |
| `<title>` | 給人看的中文標題。**不會**進 React 代碼 |
| `<S> steps` | 該章 step 總數 |
| `~<T>s` | 該章口播總估時（中文 ~ 4 字/秒） |

合法 id：`coldopen`、`hook`、`why-good`、`why-good-text-render`。
不合法：`why_good`（用連字符）、`Hook`（小寫）、`第一章`（拉丁字符）。
...
```
- <類型>：<具體內容> —— <來源 article §X / Lxx 或簡注>
```

> **沒 article（用戶直接給 script）**：信息池退化爲"主動設計畫面信息
> 密度"——靠數字 / 對比 / 元數據等讓畫面比口播信息密。可以列"畫面
...
| **屏幕內容** | 一句話講清楚這一步舞臺上有什麼：hero / 標語 / 數據 / 裝飾元素。**≤ 1 行**，再多就該拆 step |
| **不寫動畫** | 寫死 = 翻譯機化（詳見本文件頂部框） |
...
完整文本回 `script.md`。`outline.md` 章節 = `script.md` 中兩個明顯
主題切換之間的段落。

> 音頻合成（[`AUDIO.md`](AUDIO.md)）會**回到 `script.md`** 切分完整
> 文本，**不**用 outline 節選。
...
| 對象 | 規則 | 示例 |
|---|---|---|
| 章節 id | 小寫 + 連字符 | `coldopen`, `why-good` |
| 章節文件夾 | `0N-<id>` | `src/chapters/01-coldopen/` |
| 章節組件 | PascalCase | `Coldopen.tsx`, `WhyGood.tsx` |
| 章節 CSS 類前綴 | 章節縮寫（避免跨章衝突） | `.cd-` / `.wg-` / `.mg-` |
| 音頻子目錄 | `<id>/` | `public/audio/coldopen/` |
| 音頻文件 | `<step-N>.mp3` (1-indexed) | `public/audio/coldopen/1.mp3` |

---

## 章節切分的經驗法則

- **每章 3~8 步**。少於 3 步太薄；多於 8 步觀衆會忘記這章在講啥
- **總時長 ÷ 30 秒** ≈ 章節數（一章約 30~60 秒講完）
- **每章 = 一個聚焦主題**。"爲什麼強 + 怎麼用" 是兩章，不是一章
- **章節邊界 = 口播稿裏講者會換語氣 / 換主題的位置**。讀 `script.md`
  時哪裡你下意識想"咳一聲接下一段"，那裡就是章節邊界
- **慢節奏 / 長鏡頭風主題**（midnight-press / 電影感片頭）每章可少到
  2~3 step；**信息密集型**（科技測評 / 對比表）每章可放寬到 8~10 step
...
> **禁止**寫完直接進入 Checkpoint Plan 讓用戶對齊。
>
> **執行方式**（按能力降級）：
>
...
- [ ] 每個 step 都是**單一句屏幕內容描述**，沒有"動畫"行 / "手段"行
- [ ] 沒有任何 step 寫了具體毫秒 / 秒數（除 `(~Ts)` 口播估時）
- [ ] 每章首段都有「信息池」block，至少 3 條 article 抽取項，**每條
      必帶來源標註**（`—— 來源 article §X / Lxx`）—— 沒標註 chapter agent
      回不到原文
- [ ] **所有 step `(~Ts)` 累加 ≈ 頂部聲明的總時長**（誤差 < 10%）—— 不
      一致說明節奏規劃失真
- [ ] 章節切分符合"每章 3~8 步 / 30~60s 一聚焦主題"經驗
- [ ] 末尾「素材清單」分章節列出，✓ / ⚠️ 標註清楚
- [ ] 腳本不得包含標題、序號等非口播內容，僅包含人類正常可讀的內容
