...
| Provider | 默认 | 何时用 |
|---|---|---|
| `edge-tts` | ✓ | **推荐新手首选**：免费、无需 API key，`pip install edge-tts` 即可；支持 `--rate` / `--pitch` 调速调调 |
| `minimax`  | —— | 中文音色质量高（用 `mmx-cli`，要 MiniMax API key） |
...
- 用默认 `edge-tts`（免费，无需 key）→ 走 [2.A](#2a-用内置-edge-tts-合成默认)
- 用内置 `minimax` → 走 [2.B](#2b-用内置-minimax-合成)
- 用内置 `openai` → 走 [2.C](#2c-用内置-openai-合成)
- 想用别的 TTS / 自带 TTS → 走 [2.D](#2d-换-provider--加自定义-provider)
- 一个都没装好 → 走 [2.E](#2e-退化路径)

#### 2.A 用内置 edge-tts 合成（默认）

**最快上手**：免费、无 API key，仅需 Python 环境。

```bash
# 安装（只需一次）
pip install edge-tts

# 合成（edge-tts 是默认 provider，直接跑即可）
npm run synthesize-audio              # 增量：跳过已存在的 mp3
npm run synthesize-audio -- --force   # 全部重合成
npm run synthesize-audio -- --voice=zh-TW-YunJheNeural  # 台湾男声（默认）
npm run synthesize-audio -- --voice=zh-TW-HsiaoChenNeural  # 台湾女声
```
...
可用中文音色（执行 `edge-tts --list-voices | grep zh` 查完整列表）：

| 音色 ID | 语言 | 性别 |
|---|---|---|
| `zh-TW-YunJheNeural` | 台湾中文 | 男 |
| `zh-TW-HsiaoChenNeural` | 台湾中文 | 女 |
| `zh-CN-YunxiNeural` | 中国大陆 | 男 |
| `zh-CN-XiaoxiaoNeural` | 中国大陆 | 女 |

启动时 runner 会先调 provider 的 `tts_check`：

- edge-tts 未安装 → 报 `'edge-tts' command not found`，并打印 `pip install edge-tts`
- 安装正常则直接开始合成，无需登录 / Key
...
mmx auth login --api-key sk-xxxxx  # 在 https://platform.minimaxi.com 获取 key
...
```
如果默认 edge-tts 没就绪，可以退化到手动录制：

```
我可以：

  1. 帮你装 edge-tts（默认 provider，免费无 key）
     pip install edge-tts
