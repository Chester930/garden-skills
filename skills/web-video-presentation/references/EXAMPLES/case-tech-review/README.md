...
一篇 AI / 工具 / 產品**實測對比**類文章 → 7 章 36 步、6 分 30 秒視頻
的真實案例。

> ## ⚠️ 這是結構示意 / 歷史案例，不是抄襲模板
>
> 這個目錄的角色是讓 agent 看："**測評類視頻章節怎麼切、信息池怎麼
...
> 新寫 outline 時只寫"屏幕內容 + 關係名前綴 + 章節級信息池"，動畫
...
> 3. 雙源原則怎麼落地（hero 來自 script / 數據角標來自 article）
>
...
| 維度 | 這個案例的選擇 | 通用啓發 |
|---|---|---|
| 主題 | `midnight-press`（電影感慢鏡、blur clear、暖橙 accent、scanline） | 科技測評類適合"克制、有重量"的暗色調；避開俏皮 / 糖果色 |
| 章節切分 | 7 章：開場懸念 / 強在哪 / 哪能用 / 怎麼用好 / Skill 介紹 / Skill 模式 / 收尾 | 測評類的標準結構：鉤子 → 優點 → 場景 → 進階 → 收束 |
| 章長 | 每章 4~6 step | 測評類信息密度高，每章不超過 6 step 防止觀衆疲勞 |
| 雙源應用 | hero 標語來自 script、畫面密度（具體分數 / 投票數 / 時間戳）來自 article | 測評類 article 數據極多 —— 用 mono cue / 角標 / 數據浮層掛出來 |
...
- [`outline-snippet.md`](outline-snippet.md) —— 前 2 章完整節選（5 + 5 step），
  展示雙源原則在 outline 裏怎麼落地

> 完整 7 章 outline 在調用此 Skill 的具體項目裏（`gpt-image2-video/outline.md`），
> 不放進 Skill 倉庫 —— 避免 Skill spec 被某一個項目內容污染。
