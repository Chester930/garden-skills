...
> 3. **保留它的"形"**（step 切分邏輯、字號關係、布局原則），**按本
>    項目的主題 + 內容換動作選型**
>
> 倒過來——先翻 EXAMPLES 選一個照搬到底 = [`../CHAPTER-CRAFT.md`](../CHAPTER-CRAFT.md)
> Part 5 第 8 條「整章只用一種入場動畫」同質化反模式（每個用戶的視頻
> 看起來像同一個模板的 N 個變奏）。
...
| 例子 | 題材 | 文件 |
|---|---|---|
| [`case-tech-review/`](case-tech-review/) | 科技測評 / 實測對比 / 跑分類視頻 | README + outline 節選 |

> 題材 case 展示**真實 outline 的樣子**（含 article 補字段如何填、
...
3. 翻 `chapter.tsx` 看實現：JSX 結構、`step` 切分、用了哪些組件 / 類名
...
| 主導動作 | mask reveal + 印章砸下 | + Canvas 粒子從屏幕外匯聚成 hero 字 |
...
**判斷標準**：
- 如果不同 step 的主導動作夠多樣（PPT 警報通過 [`../CHAPTER-CRAFT.md`](../CHAPTER-CRAFT.md)
...
Part 3 視覺工具箱（CSS / SVG / Canvas / JS 全棧）裏覆蓋了；按 anchor
的"形"組合即可。
