...
> 每 step 只填一個槽位 + 位置不重排）——保留這個**結構**，**按本項目的
...
視頻中段最常用的章節類型，**最容易翻車成 PPT** —— 這是爲什麼需要 anchor。
...
| 3 | "02" 填充；01 已激活變次級；其餘仍 ghost |
| ... | 當前槽位填充；之前的激活降級；之後的 ghost |

## CHAPTER-CRAFT.md Part 0 原則 8 的核心實現

> "布局不重排，只是單元格內容變化"

整個章節只有**一個網格布局**，N 個槽位的 React 節點位置完全不變。
變的只是每個槽位的內容狀態（ghost / active / past）。這樣：
- 單元格不會重排 → 視覺穩
- 每點一次只有"一個槽位變化" → 觀衆視線明確鎖定新揭示的項

**反模式**：每點一次重新渲染整個布局 → 已揭示的項也跟着抖動 / 重新
入場 → 觀衆不知道該看哪。
