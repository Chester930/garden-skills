...
打斷視頻的視覺連貫性，錄屏時看起來像很硬的剪輯。如果想要"暗一點的氛圍"
段落，請在**同一調色板內**降對比、收聚光，而不是翻轉表面色。

主題 = 一組 CSS 設計 token + 一個 `theme.json` 元數據。
...
主題**不只管**顏色和字體，但其他維度（hero 數字、分割線、卡片、舞臺
裝飾）通過 **primitive class**（`.hero-num` / `.rule` / `.card` /
...
| 維度                       | 主題怎麼管                                                        |
| -------------------------- | ----------------------------------------------------------------- |
| **調色板**                 | shell / surface 階梯、text 階梯、accent + 透明度衍生              |
...
| `chalk-garden`    | 深石板黑板。Patrick Hand 全場手寫，粉筆黃 accent。**2px 虛線 rule** 是籤名。film grain（overlay）+ vignette。襯線帶 chalk text-shadow。手繪節奏。                                                                |
| `terminal-green`  | 80 年代磷光終端。純黑 + JetBrains Mono only + 0px 直角。**CRT 掃描線**貼在舞臺上。文字帶磷光 text-shadow。利落線性動效（180/400/650ms）。hero 數字：等寬帶發光。                                                  |
| `blueprint`       | 工程藍圖。深海軍藍 + 繪圖青 + IBM Plex Mono。**2px 虛線青色 rule + 60px 製圖網格**是籤名。hero 數字：等寬青色。等寬配對營造技術 / 藍圖感。                                                                       |
| `dark-botanical`  | 高級感編輯暗底 —— 時尚刊物 / 博物館圖錄。近黑 + 暖陶 / 玫粉 / 鎏金疊層。Cormorant italic + IBM Plex Sans。**柔光暈染（blurred light pool）作爲籤名**。慢速電影感節奏（1.7s）。140×100 padding。              |
| `neon-cyber`      | 賽博朋克未來派。深海軍底 + 電光青 + 玫紅雙霓虹。Clash Display + Satoshi。**青色發光網格 + 雙色霓虹描邊（cyan + magenta text-shadow）**是籤名。snappy 節奏（380/650ms）。                                       |
| `bold-signal`     | hero pitch-deck 暗底。Archivo Black + Space Grotesk。大橙色焦點色卡 + 制表數編號。**對角線深色漸變 + 大字標語**是籤名。punchy 節奏（420/680ms）。                                                                |
| `creative-voltage`| 復古朋克創意工作室。飽和電光藍底 + 霓虹黃強調。Syne + Space Mono。**halftone 網點 + 偏移霓黃陰影**是籤名。punchy + 能量節奏（450/720ms）。                                                                       |
...
| `vintage-editorial`  | 俏皮編輯奶油底。Fraunces italic + Work Sans + 暖陶 accent。**細線幾何疊層（圓 + 線 + 點）**是籤名。有性格、會說話，像專欄作家。中速帶輕微 overshoot。                                                            |
| `pastel-dream`       | 友好柔光。柔粉藍灰底 + 奶油卡 + 鼠尾草綠。Plus Jakarta Sans。**大圓角（20px）+ 右側多色 pill 色條**是籤名。soft springy（520/820ms）。                                                                          |
| `split-canvas`       | 雙拼畫布 —— 蜜桃 + 薰衣草 50/50 硬切分。Outfit + 玫紅 accent。**屏幕本身就是雙色底**，章節自由在哪一側落內容。playful 中速（480/780ms）。                                                                       |
| `electric-studio`    | 企業電光藍。淨白底 + 單一電光藍 + Manrope。**貼底 4px 電藍條**作爲籤名，B2B / 路演 / 財報場景的清晰自信。punchy 節奏（420/700ms）。                                                                              |
...
| `kraft-paper`        | 牛皮紙 —— **深棕當墨** + 牛皮米。Fraunces + Source Serif + 紫銅 accent。老筆記本 / 老信封感。**粗暖紙紋**是籤名。慢速 tactile（1.55s）。                                                                       |
| `dune`               | 沙丘 —— **炭褐當墨** + 沙底 + 幾乎無 accent（muted clay）。Inter display + Source Serif 正文。**無裝飾 + 極寬 padding（140×100）**是籤名。建築手冊 / 畫廊感。最慢節奏（1.75s）。                                |
...
刷新 dev server。完成。章節代碼一行沒動。

如果切換後某章節看起來有問題，那是該章節在某處硬編碼了顏色 / 字體 /
尺寸，而不是用語義 token。去找出來 —— bug 在章節裏，不在主題裏。

---

## 完整 token 契約

`base.css` 給**性格 token 都準備了合理的默認值**。主題的 `tokens.css`
只需要覆蓋**調色板 + 字體 + 性格旋鈕 + 裝飾**這四類。

> **base.css 裏的字號 / 間距 / 時長尺度只供 primitive class 自己用**
...
| `--surface-3` | 最裡層 —— surface-2 裏再嵌一層時用                  |
...
這些默認是 no-op；主題選擇性啓用。裝飾畫**在舞臺上**（pattern 用
`stage-frame::after`，vignette 用 `stage-frame::before`），所以會被屏幕
錄製器捕捉到。
...
| 教室 / 解說                             | `chalk-garden`        |
| 終端 / 黑客 / 復古 CRT                  | `terminal-green`      |
| 紀錄片 / 嚴肅 / 新聞                    | `newsroom`            |
| 工程 / 藍圖 / 技術                      | `blueprint`           |
| 現代主義 / 布魯塔利斯特 / 宣言          | `bauhaus-bold`        |
| 獨立 / 玩味 / zine                      | `sunset-zine`         |
| 精煉 / 安靜 / 印刷                      | `monochrome-print`    |
| 高級感暗底 / 時尚 / 博物館圖錄          | `dark-botanical`      |
| 賽博朋克 / 未來感 / AI / web3           | `neon-cyber`          |
| 俏皮編輯 / 有聲音的博主 / 文化隨筆      | `vintage-editorial`   |
| 柔粉 / 友好 / onboarding / 女性向       | `pastel-dream`        |
| 雙色分屏 / 對照 / 辯論                  | `split-canvas`        |
| pitch deck / 大字宣言 / 焦點色卡         | `bold-signal`         |
| B2B / 企業 / 投資人路演                 | `electric-studio`     |
| 復古朋克 / 創意工作室 / 設計周          | `creative-voltage`    |
| 學術 / 研究 / 中國當代文化              | `indigo-porcelain`    |
| 自然 / 可持續 / 戶外 / 紀錄             | `forest-ink`          |
| 文學 / 懷舊 / 書評 / 手工藝             | `kraft-paper`         |
| 建築 / 藝術展覽 / 高端畫廊              | `dune`                |
| 瑞士國際主義 / Helvetica / 信息驅動設計 | `swiss-ikb`           |
...
- 深色主題裏 `--shell` **比 `--surface` 更深 / 更飽和**；淺色主題裏
...
- 挑**一個設計籤名**重重發力：虛線 rule、粗黑邊、掃描線、紙紋、glass
  slab。別同時疊三個。
...
| 字段 | 必填 | 取值 | 決定什麼 |
|---|---|---|---|
| `id` / `name` / `nameZh` | ✓ | 字符串 | 主題標識 |
| `description` / `descriptionZh` | ✓ | 一句話 | Checkpoint Plan 列清單時的簡介 |
| `mood` | ✓ | 標籤數組 | 模糊匹配用 |
| `bestFor` | ✓ | 場景數組 | Checkpoint Plan 智能推薦時的命中點 |
| `preview` | ✓ | 4 色對象 | Checkpoint Plan 列清單時的視覺預覽 |

> **主題不再約束動畫選型 / 時長 / 字號 / emoji**。視覺風格由 `tokens.css`
> 的顏色 / 字體 / 字號 token 決定，動畫 / 節奏 / 視覺演示完全交給 chapter
> agent 在每章實現時按內容自由發揮，避免主題字段過早限制創造力。
>
> 風格審美約束（不要紫粉漸變、不要 emoji 裝飾、不要假數據等）由
...
- **章節 CSS 硬編碼 hex 顏色 / 字體名** —— 缺哪個色彩 / 字體語義就在
  契約裏補一個，給所有主題加上（注意：**字號 / 間距 / 時長**硬編碼不算
  反模式，章節按內容自由設計）
- **演示中途切換主題** —— 選一個，一以貫之
- **第二個 accent 色** —— 只能有一個。用尺度 + 字重做層級
- **在組件層 override 主題 token**（顏色 / 字體 / 性格籤名）—— 只在
  `:root` 裏覆蓋。一次性的顏色需求 = 提一個派生 token，讓所有主題都
  提供自己的值
- **依賴主題的 TSX 條件分支** —— 章節必須主題無關。布局依賴明 vs 暗
  = 布局脆弱，修布局
- **一個主題疊三個設計籤名** —— 選 ONE 個（虛線 rule / 掃描線 /
