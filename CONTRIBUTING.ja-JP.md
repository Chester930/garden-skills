...
│   ├── update-readme.mjs                ← README 內のダウンロードリンクを書き換え
...
このリポジトリのすべてのスキルは、同じ最小限の形狀に従います:
...
├── README.zh-CN.md     ← 人間向けの中國語ドキュメント
...
description: このスキルが何をして、いつ使うかを明確に説明する文章。
              エージェントはこれを使ってスキルを読み込むかどうかを判斷します。
---
...
[Anthropic 公式のサンプル](https://github.com/anthropics/skills) を參照してください。
...
2. すべてのルート多言語 README で、新しいスキルの「Links:」/
   「鏈接：」行の末尾にインラインの DOWNLOAD マーカーを追加します（先頭に ` · ` を付ける）:
   ```markdown
...
# 1 つまたは複數のスキルのバンプを事前に選択（殘りはプロンプト）。
npm run release -- \
...
各スキルは [SemVer](https://semver.org/) に従って**獨立して**バージョン管理されます。

| 変更 | バンプ |
|---|---|
| 誤字修正、新しいオプションのリファレンス、`SKILL.md` のマイクロエディット | **patch** |
| `SKILL.md` 內のワークフロー変更、`references/` の再構成、新しい必須ステップ | **minor** |
...
手動で編集してから、自分でタグをプッシュしてください（[手動リリースのフォールバック](#手動リリースのフォールバック)を參照）。
...
すべての PR と、`skills/**`、`scripts/release/**`、いずれかの多言語 README に觸れる
...
`releases/latest/download/<asset>` は**リポジトリ全體**の最新リリースに
リダイレクトしますが、これは各スキルが獨立してリリースされるマルチスキル
...
- **なぜリポジトリ全體のバージョニングではなく、スキル別の SemVer なのか？**
  スキルは非常に異なるペースで進化します。それらを結合すると、ダウンストリームの
  ピン留めにペナルティを課すことになります。
- **なぜ rolling-latest タグがないのか？**
  GitHub は既にリポジトリ內の最新リリースに対して `releases/latest/download/<asset>` を
  提供しており、README 內のスキル別バージョン固定 URL はリリースごとに自動的に
...
- **なぜ npm パッケージを公開しないのか？**
  ...
  理解しているため、プライベート CLI として同じインストール UX を再実裝しても
  エコシステムを斷片化するだけです。
- **なぜ npm 依存関係がゼロなのか？**
